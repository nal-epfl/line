...
then
	...
	ssh $REMOTE_USER@$REMOTE_HOST -p $REMOTE_PORT "sh -l -c \"set -x && cd $REMOTE_DIR && touch $PF_RING && cd $PF_RING && find . -exec touch {} \;\""
	rm -f stdoutfifo
	rm -f stderrfifo
...
ssh $REMOTE_USER@$REMOTE_HOST -p $REMOTE_PORT "sh -l -c \"set -x && cd $REMOTE_DIR && touch malloc_profile && cd malloc_profile && find . -exec touch {} \;\""
rm -f stdoutfifo
rm -f stderrfifo
mkfifo stdoutfifo || exit 1
cat stdoutfifo &
...
cp $SRC_DIR/deploycore-template.pl $SRC_DIR/deploycore.pl && chmod +x $SRC_DIR/deploycore.pl
